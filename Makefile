...
IMG ?= quay.io/cldmnky/krcrdr:$(VERSION)
...
build: manifests generate fmt vet ## Build manager binary.
	go build -o bin/krcrdr

.PHONY: run
run: manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/*.go --controller --api --frontend

.PHONY: container-build
container-build: build-multiarch ## Build container image with krcrdr.
...
build-multiarch: gox generate fmt vet ## Build manager binary for multiple platforms.
	${GOX} -osarch=${RELEASE_IMAGE_PLATFORMS} -output="bin/release/{{.OS}}/{{.Arch}}/krcrdr"

.PHONY: container-publish
container-publish: container-build container-push ## Build and push container image with the manager.
...
.PHONY: oapi-codegen
oapi-codegen: $(OAPI-CODEGEN) ## Download oapi-codegen locally if necessary.
